...
import { Grid, ProgressRing, Typography } from '@dharshi/ui-toolkit';
...
import { BallerinaConstruct, BallerinaModuleResponse } from "@dharshi/ballerina-core";
...
export function Marketplace(props: MarketplaceProps) {
    ...
    const fetchModulesList = async (page?: number) => {
        ...
        console.log("fetch modules req sent: ", currentFilePath);
        const response: BallerinaModuleResponse = await props.fetchModulesList(queryParams, currentFilePath,
            rpcClient);
        console.log("fetch modules res recieved: ", Object.keys(response));
        localModules.current.clear();
        console.log(response.local?.length);
        console.log(response.central?.length);
    ...
    const renderModulesList = (modulesListTitle: string, modules: ReactNode[]): ReactNode => {
        console.log("rendering modules: ", modulesListTitle, " ", modules) // prints all the modules corectly 
        return (
            <>
                {shortName !== "Triggers" ? (
