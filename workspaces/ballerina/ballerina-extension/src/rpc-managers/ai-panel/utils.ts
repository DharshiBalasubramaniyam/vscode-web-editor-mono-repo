import { FunctionDefinition, ModulePart, QualifiedNameReference, RequiredParam, STKindChecker } from "@dharshi/syntax-tree";
import { AI_EVENT_TYPE, ErrorCode, FormField, STModification, SyntaxTree, AttachmentResult } from "@dharshi/ballerina-core";
...
export const BACKEND_API_URL_V2 = getPluginConfig().get('rootUrl') as string || "https://dev-tools.wso2.com/ballerina-copilot/v2.0";
...
export async function getAccessToken(): Promise<string> {
    let token:string = await balExtInstance.context.secrets.get("BallerinaAIUser");
    if (token) {
        return token;
    }
...
export async function refreshAccessToken(): Promise<string> {
    ...
    const refresh_token = await balExtInstance.context.secrets.get('BallerinaAIRefreshToken');
    if (!refresh_token) {
        ...
        try {
            ...
            const response = await fetch(`https://api.asgardeo.io/t/${AUTH_ORG}/oauth2/token`, {
                method: 'POST',
            ...
            await balExtInstance.context.secrets.store('BallerinaAIUser', newAccessToken);
            await balExtInstance.context.secrets.store('BallerinaAIRefreshToken', newRefreshToken);
