...
import { ModulePart, STKindChecker, STNode } from "@dharshi/syntax-tree";
...
import { MODIFIYING_ERROR, PARSING_ERROR, UNAUTHORIZED, UNKNOWN_ERROR } from "../../utils/ai/errorCodes";
import { getFunction, handleLogin, handleStop, isErrorCode, isLoggedin, notifyNoGeneratedMappings, processMappings } from "./utils";
import { StateMachine, updateView } from "../../state-machine";
import { NOT_SUPPORTED } from "../../extended-language-client";
import { balExtInstance } from "../../extension";
export let hasStopped: boolean = false;

export class AiPanelRpcManager
// implements AIPanelAPI 
{
    ...
    async generateMappings(params: GenerateMappingsRequest): Promise<GenerateMappingsResponse> {
        console.log(">>> Generate mappings called");
        ...
        if (fn && fn.source !== oldSource) {
            ...
            const noOfLines = (new TextDecoder().decode(fileContent)).split('\n');
            const lastLine = noOfLines[noOfLines.length - 1];
            const lastLineLength = lastLine.length;
            const edit = new WorkspaceEdit();
            edit.replace(Uri.parse(filePath), new Range(new Position(0, 0), new Position(noOfLines.length - 1, lastLineLength)), fn.source);
    ...
    async notifyAIMappings(params: NotifyAIMappingsRequest): Promise<boolean> {
        ...
        if (result === undoAction) {
            ...
            const lastLineLength = lastLine.length;
            const edit = new WorkspaceEdit();
            edit.replace(Uri.parse(filePath), new Range(new Position(0, 0), new Position(noOfLines.length - 1, lastLineLength)), source);
            await workspace.applyEdit(edit);
            updateView();
        }
