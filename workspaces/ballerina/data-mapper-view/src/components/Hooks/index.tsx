...
import { BallerinaProjectComponents } from "@dharshi/ballerina-core";
...
import { FunctionDefinition, ModulePart } from '@dharshi/syntax-tree';
import {
    ...
    const fetchProjectComponents = async () => {
        try {
            const componentResponse = await langServerRpcClient.getBallerinaProjectComponents({
                documentIdentifiers: [
                    {
                        uri: URI.parse(fileName).toString(),
                    }
                ]
            })
